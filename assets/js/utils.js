  ...
  if (url) {
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          `An error occurred while attempting to fetch webmentions from ${url}. Status code: ${response.status}.`,
        );
      }
    } catch (error) {
      console.error(
        `An error occurred while attempting to fetch webmentions from ${url}.`,
        error,
      );
    }
  } else {
    console.error(
      "No endpoint specified Ensure that the endpoint attribute is set with a value pointing to the appropriate endpoint for fetching webmention entries.",
    );
  }
